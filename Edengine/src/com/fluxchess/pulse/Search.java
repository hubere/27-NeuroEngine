...
import com.fluxchess.pulse.MoveList.MoveVariation;
...
final class Search implements Runnable {
	...
	// We will store a MoveGenerator for each ply so we don't have to create
	// them
	...
	static Search newPonderSearch(IProtocol protocol, Board board, long whiteTimeLeft, long whiteTimeIncrement,
		...
		// Don't use all of our time. Search only for 95%. Always leave 1 second
		// as
		// buffer time.
		long maxSearchTime = (long) (timeLeft * 0.95) - 1000L;
		if (maxSearchTime < 1) {
			// We don't have enough time left. Search only for 1 millisecond,
			// meaning
			// get a result as fast as we can.
			maxSearchTime = 1;
		}

		// Assume that we still have to do movesToGo number of moves. For every
		// next
	...
	public void run() {
		...
		for (int depth = initialDepth; depth <= searchDepth; ++depth) {
			...
			// Sort the root move list, so that the next iteration begins with
			// the
			// best move first.
			rootMoves.sort();
	...
	private int quiescent(int depth, int alpha, int beta, int ply, boolean isCheck) {
		...
		// Only generate capturing moves or evasion moves, in case we are in
		// check.
		MoveGenerator moveGenerator = moveGenerators[ply];
		MoveList moves = moveGenerator.getMoves(board, depth, isCheck);
		for (int i = 0; i < moves.size; ++i) {
