...
import com.comed.edengine.tools.fluxchess.pulse.EdEvaluation;
...
public class Search implements Runnable {
	...
	protected final EdEvaluation evaluation = new EdEvaluation();

	// We will store a MoveGenerator for each ply so we don't have to create them
	// in search. (which is expensive)
	protected final MoveGenerator[] moveGenerators = new MoveGenerator[Depth.MAX_PLY];
	...
	protected boolean doTimeManagement = false;

	// Search parameters
	protected final MoveList rootMoves = new MoveList();
	private boolean abort = false;
	protected long startTime = 0;
	protected long statusStartTime = 0;
	...
	protected final MoveVariation[] pv = new MoveVariation[Depth.MAX_PLY + 1];

	/**
	 * This is our search timer for time & clock & ponder searches.
	 */
	final class SearchTimer extends TimerTask {
		@Override
		public void run() {
			timerStopped = true;
	...
	static Search newDepthSearch(IProtocol protocol, Board board, int searchDepth) {
		...
		if (board == null)
			throw new IllegalArgumentException();
		if (searchDepth < 1 || searchDepth > Depth.MAX_DEPTH)
			throw new IllegalArgumentException();
	...
	static Search newClockSearch(IProtocol protocol, Board board, long whiteTimeLeft, long whiteTimeIncrement,
			long blackTimeLeft, long blackTimeIncrement, int movesToGo) {
		Search search = newPonderSearch(protocol, board, whiteTimeLeft, whiteTimeIncrement, blackTimeLeft,
	...
	static Search newPonderSearch(IProtocol protocol, Board board, long whiteTimeLeft, long whiteTimeIncrement,
			long blackTimeLeft, long blackTimeIncrement, int movesToGo) {
		if (protocol == null)
			throw new IllegalArgumentException();
		if (board == null)
		...
		if (whiteTimeLeft < 1)
			throw new IllegalArgumentException();
		if (whiteTimeIncrement < 0)
			throw new IllegalArgumentException();
		if (blackTimeLeft < 1)
			throw new IllegalArgumentException();
		if (blackTimeIncrement < 0)
			throw new IllegalArgumentException();
		if (movesToGo < 0)
			throw new IllegalArgumentException();
	...
	protected Search(IProtocol protocol, Board board) {
	...
	protected void updateSearch(int ply) {
		++totalNodes;
